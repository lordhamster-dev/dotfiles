---
name: gpt-image-2
description: 面向 GPT Image 2 的图像生成 / 编辑技能。作为 Host-Native 或 Advisor 提示词工程 Skill 使用：宿主有图像工具时，把渲染好的 prompt 交给宿主 Agent 自带工具出图；宿主无图像工具时，退化为高质量 prompt 顾问。涵盖 18 大类、80+ 个结构化模板，覆盖海报 / UI / 产品 / 信息图 / 学术图 / 技术架构图 / 漫画 / 头像 / 流程板 / 电影分镜 / IP 周边 / 编辑工作流等场景。
---

# GPT Image 2

这是一个面向 GPT Image 2 的聚焦型提示词工程技能。在两种运行环境下工作：

- **Host-Native 模式**：当前宿主 Agent 自带图像生成工具或图像 MCP，本 Skill 负责选模板、补字段、渲染 prompt，再把 prompt 交给宿主工具出图。
- **Advisor 模式**：宿主没有图像生成工具，本 Skill 退化为高质量 prompt 撰写顾问，保存并展示可复用 prompt。

本文件保留：运行模式、技能结构、保存 / 命名规则、模板索引、模式感知工作流。详细模板全部放在 `references/`，分层组织：
...
本 Skill 不提供本地生图能力。先自检当前宿主是否有图像生成工具，再决定进入 Host-Native 或 Advisor。

### Mode A · Host-Native 委托宿主出图

**触发条件**：当前宿主 Agent 自带图像生成工具或图像 MCP。
...
1. 按“选模板 → 填字段 → 渲染最终 prompt”的流程走。
2. 直接调用宿主自带的图像工具，把渲染好的 prompt 作为输入。
3. 如用户希望，可顺手把 prompt 文件保存到 `gpt-image-2/prompt/`；图片去向由宿主工具决定。

### Mode B · Advisor 纯提示词顾问

**触发条件**：宿主 Agent 没有任何图像生成工具。

**行为**：

1. 按“选模板 → 填字段 → 渲染最终 prompt”流程走，缺信息就问用户。
2. 把最终 prompt **直接打印给用户** + 保存一份到 `gpt-image-2/prompt/<task-slug>-<timestamp>.md`。
...
| 条件           | 模式  | 本 Skill 是否直接出图？ | 落盘 prompt？ | 落盘图片？ |
| -------------- | ----- | ----------------------- | ------------- | ---------- |
| 宿主有图像工具 | **A** | ❌（用宿主工具）        | 可选          | 由宿主决定 |
| 宿主无图像工具 | **B** | ❌                      | ✅ 必须       | ❌（无法） |

### 模式不确定时

- 如果你判断不清自己是 A 还是 B，**直接问用户一句**：“是用你环境里的图像工具出图，还是只要我写好提示词？”
...
- `references/`：分层结构化提示词模板（Host-Native / Advisor 两种模式都用）

## 环境变量

本 Skill 不再读取本地出图相关环境变量，也不直接调用 OpenAI 兼容接口。宿主图像工具如需 API key / base URL / model，请按宿主工具自身规则配置。

## 默认输出目录

如果用户没有明确指定输出路径，统一使用当前工作区下的：

- 提示词目录：`gpt-image-2/prompt/`（**两种模式都建议用**，方便复用与版本管理）
- 图片目录：不由本 Skill 固定；Mode A 由宿主工具决定，Mode B 不产生图。
...
- 提示词：`gpt-image-2/prompt/<task-slug>-<timestamp>.md`
...
- `gpt-image-2/prompt/live-commerce-ui-20260424-153045.md`
- `gpt-image-2/prompt/vr-headset-exploded-view-20260424-153102.md`

## Prompt 保存规则

| 模式   | 是否必须保存 prompt | 说明                                     |
| ------ | ------------------- | ---------------------------------------- |
| Mode A | 推荐                | 默认建议保存方便复用；用户说"不用"就略过 |
| Mode B | ✅ 必须             | 用户拿走 prompt 自己执行，不落盘等于白干 |

通用规则：

1. 如果用户显式给了 prompt 文件路径，可直接使用该文件作为输入。
2. 如果用户直接给的是文本 prompt，也要先把最终 prompt 保存到 `gpt-image-2/prompt/`。
3. 如果用户显式指定了 `--prompt-output`，则尊重用户指定路径。
4. 否则使用默认命名规则自动保存。

## 图片保存规则

Mode A 由宿主图像工具决定保存方式；Mode B 不产生图片。
...
无论 A / B，前 6 步是共用的；区别只在第 7-8 步如何“出图”。

1. **自检宿主是否有图像工具**，确定 Mode A 或 Mode B。
2. 判断任务是生图还是改图。
...
7-A. **Mode A**：把最终 prompt 直接传给宿主的图像工具调用；按需保存 prompt 副本到 `gpt-image-2/prompt/`。
7-B. **Mode B**：把最终 prompt 保存到 `gpt-image-2/prompt/<task-slug>-<timestamp>.md`，并把完整 prompt 在对话中展示给用户，附一句简短的"如何使用 / 推荐工具"建议。
...
- 两种模式下，最终交给图像模型或用户的都是"渲染后的 prompt 字符串"——可以是拍平的 JSON、可以是结构化自然语言段落，按模板原样使用。
